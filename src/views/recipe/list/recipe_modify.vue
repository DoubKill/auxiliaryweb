  ...
  <div v-loading="loading" class="recipe_modify" element-loading-text="加载中...">
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="预计炼胶时间">
        <el-input-number v-model="production_time_interval" style="width:100px" :step="1" step-strictly :min="0" controls-position="right" size="mini" />
      </el-form-item>
      <!-- <el-form-item label="机台">
        <el-select
          v-model="equip_name"
      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <div class="grid-content bg-purple">
            ...
            <el-form :model="speedObj" :rules="rules" ref="ruleForm">
...
export default {
  ...
  methods: {
    ...
    removeOilRow1(row) {
      ...
      this.oil_tableData1.forEach((d, i) => {
        d.sn = i + 1
      })
    },
          ...
          if (recipe_listData['batching_details'][j]['type'] === 2) {
            let carbonItem = this.tankCarbons.find(item => {
              return (item.id === recipe_listData['batching_details'][j].material) &&
                (item.tank_no === recipe_listData['batching_details'][j].tank_no)
            })
            ...
            if (['Z07', 'Z04'].includes(this.equip_no) && recipe_listData['batching_details'][j]['line_no'] === 2) {
              ...
              if (!oilItem) { // 不在下拉选项中的数据
                oilItem = {
                  ...
                  tank_no: recipe_listData['batching_details'][j].tank_no || '',
                  provenance: recipe_listData['batching_details'][j].provenance || '',
                  _index: this.tankOils1.length
                }
                this.tankOils1.push(oilItem)
              }
              this.oil_tableData1.push({
                action_name: '投料',
    ...
    modify_material_button: async function() {
      ...
      for (var i = 0; i < popup_materialData['batching_details'].length; ++i) {
        this.ProductRecipe.push({
          ...
          material: popup_materialData['batching_details'][i]['material'],
          material_type: popup_materialData['batching_details'][i]['material_type'],
          auto_flag: popup_materialData['batching_details'][i]['auto_flag'],
          material_name: popup_materialData['batching_details'][i]['material_name'],
          actual_weight: popup_materialData['batching_details'][i]['actual_weight'],
          standard_error: popup_materialData['batching_details'][i]['standard_error']
        })
      }
    },
    ...
    saveMaterialClicked: async function() {
      ...
      for (var i = 0; i < this.ProductRecipe.length; ++i) {
        ...
        if (app.ProductRecipe[i].material_name) {
    ...
    recipe_save_return: async function() {
      ...
      if (this.sp_num) {
        ...
        for (j = 0; j < oil_tableDataArr.length; ++j) {
          ...
          if (oil_tableDataArr[j].material_name.indexOf('卸料') > -1 && this.equip_no !== 'Z04') {
            _oilNums[0]++
            oilIndex = j
          }
